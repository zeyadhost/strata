...
function buildHeightmap(rand: () => number): number[] {
  const step = 12;
  const points: number[] = [];
  for (let index = 0; index <= Math.ceil(WORLD_WIDTH / step) + 1; index++) {
    points.push((rand() - 0.5) * 6);
  }

  const map: number[] = new Array(WORLD_WIDTH);
  for (let x = 0; x < WORLD_WIDTH; x++) {
    const segment = Math.floor(x / step);
    const t = (x % step) / step;
    const smooth = t * t * (3 - 2 * t);
    const base = Math.sin(x * 0.025) * 4 + Math.sin(x * 0.007 + 0.8) * 6;
    map[x] = Math.round(LAYER_SURFACE + base + points[segment] * (1 - smooth) + points[segment + 1] * smooth);
  }
  return map;
}
...
export function generateWorld(seed: number): number[][] {
  ...
  const tiles: number[][] = Array.from({ length: WORLD_HEIGHT }, () => new Array(WORLD_WIDTH).fill(TileType.AIR));
...
export function findSpawn(tiles: number[][]): { spawnX: number; spawnY: number } {
  const middleX = Math.floor(WORLD_WIDTH / 2);
  for (let y = 0; y < WORLD_HEIGHT; y++) {
    if (tiles[y][middleX] !== TileType.AIR) {
      return { spawnX: middleX * 16 + 8, spawnY: (y - 1) * 16 };
    }
  }
  return { spawnX: middleX * 16 + 8, spawnY: 0 };
}
